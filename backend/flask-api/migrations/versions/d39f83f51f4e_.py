"""Criando a função que realiza a validação dos processos

Revision ID: d39f83f51f4e
Revises: 8cb2ef1d5d5c
Create Date: 2020-10-20 06:56:01.157257
...
revision = 'd39f83f51f4e'
down_revision = '8cb2ef1d5d5c'
branch_labels = None
depends_on = None
...
def upgrade():
    op.execute("""
        CREATE OR REPLACE FUNCTION fn_carga_tb_hist_situacao(p_cd_processo varchar(50)) RETURNS void LANGUAGE plpgsql AS $function$ begin
                ...
                from (
                        WITH RECURSIVE hist_situacao(
                            ...
                            select distinct cd_processo,
                                ...
                                (
                                    select id_situacao
                                    from sanjus.tb_desc_situacao
                                    where cd_situacao = 'I'
                                ) as id_situacao_destino,
                            ...
                            where coalesce(p_cd_processo, a.cd_processo) = a.cd_processo
                            union all
                            /* Passo recursivo:
                            ...
                            */
                            select hs.cd_processo,
                                ...
                                (
                                    case
                                        when (tv.id_situacao_origem is not null) then tv.id_situacao_origem
                                        else hs.id_situacao_destino
                                    end
                                ) as id_situacao_origem,
                                ...
                                (
                                    case
                                        when (tv.id_situacao_destino is not null) then tv.id_situacao_destino
                                        else hs.id_situacao_destino
                                    end
                                ) as id_situacao_destino,
                                ...
                                (
                                    case
                                        when (tv.id_situacao_destino is not null) then 'S'
                                        else 'N'
                                    end
                                ) as ind_valida,
                                ...
                                hs.sg_tribunal
                            from (
                                    select he1.cd_processo,
                                        he1.dt_ocorrencia,
                                        he1.id_evento,
                                        ev.ds_evento,
                                        1 as nu_autuacao,
                                        rank() over (
                                            partition by he1.cd_processo
                                            order by he1.dt_ocorrencia
                                        )::int4 as nu_seq_evento,
                                        pro.cd_classe
                                ...
                                inner join hist_situacao hs on (
                                    hs.cd_processo = he.cd_processo
                                    and hs.nu_autuacao = he.nu_autuacao
                                    and -- a próxima saída a ser processada e verificada quanto a validade é a saída posterior em ordem à já inserida na tabela recursiva
                                    (hs.nu_seq_evento + 1) = he.nu_seq_evento
                                )
                                left outer join sanjus.tb_fluxo tv on (
                                    he.id_evento = tv.id_evento
                                    and -- a origem da transição deve ser o destino da última transição já processada
